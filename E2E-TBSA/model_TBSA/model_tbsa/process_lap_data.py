...
class ProcessData():
    ...
    def read_data(self,path):
        all_labels,all_texts,all_words,boundary_labels = [],[],[],[]
        ...
        with open(path,encoding='utf-8') as reader:
            for i,line in enumerate(reader.readlines()):
                ...
                for lab in labels:
                    if lab=='O':
                        cur_boundary_labels.append(lab)
                    else:
                        cur_boundary_labels.append(lab.split('-')[0])
                boundary_labels.append(cur_boundary_labels)
                all_texts.append(cur_text)
                all_words.append(self.max_length_text(words))
                all_labels.append(labels)
        print(entity_number)
        return all_texts,all_words,all_labels,boundary_labels
    ...
    def convert_to_vocab(self,texts,labels,boundary_labels):
        texts_ids = []
        for text in texts:
            current_text_ids = []
            for tk in text:
        ...
        for label in boundary_labels:
            boundary_labels_ids.append(self.max_length_label([boundary_dict[_] for _ in label]))

        return texts_ids,labels_ids,boundary_labels_ids
...
class Evaluate():
    ...
    def calculate(self,texts,predicts,reals,ifwriter= False):
