from tf.model_lstm.process_lap_data import ProcessData,Evaluate
from tf.model_lstm.lstm import  LSTM
...
if __name__=="__main__":
    ...
    # 倒数衰减
    learning_rate = tf.train.inverse_time_decay(learning_rate=learning_rate,global_step=n_iter,decay_steps=each_epoch_batch_number,decay_rate=decay_rate,)
     # 启动模型
    ...
    for k, train_batch in enumerate(train_batches):
        ...
        _, train_loss_batch, train_aspect_results_batch, = \
            sess.run([train_op, mymodel.loss, mymodel.y_t_predict_label],
                     feed_dict={
                         mymodel.dropout: 0.5,
        ...
        if (k+1) % each_epoch_batch_number == 0 and k != 0:
            ...
            for m, dev_batch in enumerate(dev_batches):
                ...
                lr,dev_loss_batch, dev_aspect_results_batch, = \
                    sess.run([learning_rate,mymodel.loss, mymodel.y_t_predict_label],
                             feed_dict={
                                 mymodel.dropout : 1.0,
                ...
                dev_predict_labels.extend(dev_aspect_results_batch)
            dev_precison, dev_recall, dev_f1 = evalute_utils.calculate(dev_words_real, dev_predict_labels, dev_labels)
            ...
            if dev_f1 >=max_dev_f1:
                ...
                for m, test_batch in enumerate(test_batches):
                    ...
                    lr,test_loss_batch, test_aspect_results_batch, = \
                        sess.run([learning_rate,mymodel.loss, mymodel.y_t_predict_label],
                                 feed_dict={
                                     mymodel.dropout: 1.0,
                    ...
                    test_predict_labels.extend(test_aspect_results_batch)
                    test_words_real.extend(test_words_batch)
                test_precison, test_recall, test_f1 = evalute_utils.calculate(test_words_real, test_predict_labels, test_labels_ids)
